  ...
  <div>
    ...
    <div class="block md:hidden dropdown my-4">
      ...
      <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
        ...
        <li v-if="content_detail_state.main_category">
          <p class="cursor-pointer" @click="back_to_main_category">
            {{ content_detail_state.main_category?.title }}
          </p>
        </li>
        <li v-if="content_detail_state.sub_category">
          <p class="cursor-pointer" @click="back_to_sub_category">
            {{ content_detail_state.sub_category?.title }}
          </p>
        </li>
        <li v-if="content_detail_state.chado_content">
          <p class="cursor-pointer">
            {{ content_detail_state.chado_content?.title }}
          </p>
        </li>
      </ul>
    </div>

    <div class="hidden md:block breadcrumbs text-md py-4">
      <ul>
        <li>
          <p class="badge badge-lg hover:badge-outline cursor-pointer rounded-md" @click="back_to_pedia">
            首頁
          </p>
        </li>
        <li v-if="content_detail_state.main_category">
          <p class="badge badge-lg hover:badge-outline cursor-pointer rounded-md" @click="back_to_main_category">
            {{ content_detail_state.main_category?.title }}
          </p>
        </li>
        <li v-if="content_detail_state.sub_category">
          <p class="badge badge-lg hover:badge-outline cursor-pointer rounded-md" @click="back_to_sub_category">
            {{ content_detail_state.sub_category?.title }}
          </p>
        </li>
        <li v-if="content_detail_state.chado_content">
          <p class="badge badge-lg hover:badge-outline cursor-pointer rounded-md">
            {{ content_detail_state.chado_content?.title }}
          </p>
        </li>
      </ul>
